...
async function whatsAsena () {
    ...
    conn.on ('open', async () => {
        console.log(
            chalk.blueBright.italic('✅ Login KING QUEEN Information Updated!')
        );
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('credentials-updated', async () => {
        ...
        console.log(
            chalk.blueBright.italic('⬇️ දැන් ඔබට  බොට් බවිත කර හැකියාව ඇත 😌...')
        );
        if (os.userInfo().homedir !== clh.pay) return;
        // ==================== External Plugins ====================
        ...
        let EVA_ACTİON = config.LANG == 'TR' || config.LANG == 'AZ' ? '* KING QUEEN Chatbot Olarak Çalışıyor! * ♤♚♞ \n\nn_Bu modun amacı botu tam fonksiyonel බීර් yapay zeka sohbet aracına çevirmektir ._ \n_Normal මෝඩ dönmek için_ * .fulleva * _komutunu kullanabilirsiniz ._ \n ලකුණු n\n * KING QUEEN Kullandığın İçin Teşekkürler♲♻ *\n *- Eva*' : '*KING QUEEN Chatbot එකක් ලෙස වැඩ කරයි! ♠♛♔♚*\n\n_මෙම මෝඩයේ අරමුණ වන්නේ බොට් සම්පුර්ණයෙන්ම ක්‍රියාකාරී AI චැට්බෝට් එකක් බවට පත් කිරීමයි._\n_ඔබට සාමාන්‍ය ප්‍රකාරයට ආපසු යාමට_ *.fulleva off* _command භාවිතා කළ හැක._\n\n*KING QUEEN භාවිතා කිරීමට ස්තූතියි☠☠  💌*\n    *- Eva*'
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '* KING QUEEN public ලෙස  ක්‍රියා කරයි! ♠☠♛♞♚ * \n\n_කරුණාකර මෙහි ප්ලගිනය කිරීමට උත්සාහ නොකරන්න. මෙය ඔබගේ LOG අංකයයි ☠☠♞._\n_ඔබට ඕනෑම සංවාදයකදී ගමනාගමන උත්සාහ කළ හැක :) සංසන්දනය කිරීම සඳහා * _.setvar WORK_TYPE: public _ * ගමනාගමනය භාවිතා කරන්න. * \n\n * KING QUEEN  භාවිතා කිරීමට ස්තූතියි ☠☠♠♚♛♻♲ *', MessageType.text);
                }
                await git.fetch();
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') { 
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '** KING QUEEN private  ලෙස  ක්‍රියා කරයි! ♠☠♛♞♚ * \n\n_කරුණාකර මෙහි ප්ලගිනය කිරීමට උත්සාහ නොකරන්න. මෙය ඔබගේ LOG අංකයයි ☠☠♞._\n_ඔබට ඕනෑම සංවාදයකදී ගමනාගමන උත්සාහ කළ හැක :) සංසන්දනය කිරීම සඳහා * _.setvar WORK_TYPE: public _ * ගමනාගමනය භාවිතා කරන්න. * \n\n * KING QUEEN  භාවිතා කිරීමට ස්තූතියි ☠☠♠♚♛♻♲ **', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '\n** KING QUEEN private ලෙස  ක්‍රියා කරයි! ♠☠♛♞♚ * \n\n_කරුණාකර මෙහි ප්ලගිනය කිරීමට උත්සාහ නොකරන්න. මෙය ඔබගේ LOG අංකයයි ☠☠♞._\n_ඔබට ඕනෑම සංවාදයකදී ගමනාගමන උත්සාහ කළ හැක :) සංසන්දනය කිරීම සඳහා * _.setvar WORK_TYPE: private _ * ගමනාගමනය භාවිතා කරන්න. * \n\n * KING QUEEN  භාවිතා කිරීමට ස්තූතියි ☠☠♠♚♛♻♲ *', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                if (error.message.includes('URL')) {
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*🙇🏻 සමාවන්න, මට මෙම දෝෂය කියවීමට නොහැකි විය! 🙇🏻*' +
